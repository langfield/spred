...
if [ "$(whoami)" != "mckade" ]; then
    srun -J snow -w vibranium --mem 10000 -c 4 python3 optimize.py --dataset ${DATASET} --gpst_model ${GPST_MODEL} --output_dir ${MODEL_ROOT} --save_freq ${SAVE_FREQ} --timeout ${TIMEOUT}
else
