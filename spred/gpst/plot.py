...
def graph(dfs, ylabels, filename, column_counts, phase, save_path):
    ...
    prop4 = fm.FontProperties(fname=os.path.join(cur_file_dir, 'fonts/Apercu.ttf'),
                              size=legend_size)
    ...
    xfig_trans = transforms.blended_transform_factory(fig.transFigure,
                                                      transforms.IdentityTransform())
    yfig_trans = transforms.blended_transform_factory(transforms.IdentityTransform(),
                                                      fig.transFigure)
